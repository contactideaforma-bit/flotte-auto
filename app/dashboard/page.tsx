...
export default function Dashboard() {
  ...
  const dotColor = (days: number | null) => {
        ...
        <div style={{ flex: 1, padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            ...
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
              <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--border)', fontSize: 13, fontWeight: 500 }}>
                Prochaines alertes
              </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  ...
                  <tbody>
                    {vehicles.map(v => {
                      ...
                      const ck = (ok: boolean) => (
                        <span style={{ fontSize: 16, color: ok ? 'var(--ok)' : '#ccc' }}>{ok ? '✓' : '✗'}</span>
                      )
                      return (
                        ...
                        >
                          ...
                          <td style={{ padding: '10px 12px' }}><StatusBadge status={status} days={days} /></td>
                          <td style={{ padding: '10px 12px' }}>
                            <div style={{ display: 'flex', gap: 4 }}>
                              <button onClick={() => { setEditVehicle(v); setModalOpen(true) }}
                                style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', padding: '3px 5px', borderRadius: 5, cursor: 'pointer' }} title="Modifier">
                                <Pencil size={14} />
                              </button>
                              <button onClick={() => setDelId(v.id)}
                                style={{ background: 'none', border: 'none', color: 'var(--danger)', padding: '3px 5px', borderRadius: 5, cursor: 'pointer' }} title="Supprimer">
                                <Trash2 size={14} />
                              </button>
                            </div>
                          </td>
